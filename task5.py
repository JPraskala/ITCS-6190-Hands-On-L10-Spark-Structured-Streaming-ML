...
windowed_features = windowed_df.withColumn("hour_of_day", hour(col("window.start"))) \
                               .withColumn("minute_of_hour", minute(col("window.start"))) # Replace None with your implementation
...
feature_df = assembler_inference.transform(windowed_features).select("window", "avg_fare", "features") # Keep the full window struct for later selection
